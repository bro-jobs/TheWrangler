...
namespace ff14bot.NeoProfiles.Tags
{
    ...
    internal class UserDialogForm : Form
    {
        ...
        public abstract class ExpiryActionHandler : EnumBaseType<ExpiryActionHandler>
        {
            ...
            static ExpiryActionHandler()
            {
                HandleInputDisabled_BotStop = new InputDisabled_BotStop();
                HandleInputDisabled_Continue = new InputDisabled_Continue();
                HandleInputDisabled_EnableInput = new InputDisabled_EnableInput();
                HandleInputEnabled_BotStop = new InputEnabled_BotStop();
                HandleInputEnabled_Continue = new InputEnabled_Continue();
            }
    ...
    internal class AsyncCompletionToken : IDisposable
    {
        ...
        public void WaitForComplete()
        {
            if (!IsComplete)
            {
                var whut= _actResult.Result;
            }
        }


        private Task<bool> _actResult;
