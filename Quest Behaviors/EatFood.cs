...
{
    [XmlElement("EatFood")]
    public class EatFood : ProfileBehavior
    {
        ...
        protected override void OnStart()
        {
            ...
            if (HqOnly)
            {
                ...
                {
                    TreeRoot.Stop("HqOnly and we don't have any Hq food in the inventory with id " + ItemId);
                    return;
                }
            }
            else if (NqOnly)
            {
                ...
                {
                    TreeRoot.Stop("NqOnly and we don't have any Nq food in the inventory with id " + ItemId);
                    return;
                }
            }
            else
        ...
        private async Task<bool> Eatfood()
        {
            ...
            {
                var auraInfo = Core.Player.GetAuraById(48);
            ...
            if (shouldEat)
            {
                ...
                if (!alreadyPresent)
                {
                    ...
                    await Coroutine.Wait(Timeout.InfiniteTimeSpan, () => Core.Player.HasAura(48));
                }
                else
                {
                    Log("Waiting until the duration is refreshed");
                    await Coroutine.Wait(Timeout.InfiniteTimeSpan, () => Core.Player.GetAuraById(48).TimespanLeft.TotalMinutes > MinDuration);
                }
